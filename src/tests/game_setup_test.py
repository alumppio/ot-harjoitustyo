...
class TestGameSetup(unittest.TestCase):
    ...
    def test_name_setup_loop_typing(self):
        test_name = 'TESTT'
        event = pygame.event.Event(pygame.KEYDOWN, {'key': 715517, 'unicode':'t'})
        pygame.event.post(event)
        self.game_setup.name_setup_loop(Player(), 1)
    ...
    def test_players_amount_typing(self):
        event_1 = pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_BACKSPACE})
        pygame.event.post(event_1)
        event_2 = pygame.event.Event(pygame.KEYDOWN, {'key': 1234, 'unicode':'2'})
